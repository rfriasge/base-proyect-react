...
export const Crud = () => {
    ...
    const header = (
        <div className="table-header">
            ...
            <h5 id="id_label_search" className="p-m-0">Listado de Productos</h5>
            <Button label="Export Excel" icon="pi pi-upload" className="p-button-help" onClick={exportCSV}  style={{marginRight:"10px"}}/> 
            <Button  id="btn_new" label="Agregar" icon="pi pi-plus" onClick={openNew} />
            <span id="text_search"  className="p-input-icon-left">
               <i className="pi pi-search" />
                <InputText id="id_text_search" type="search" onInput={(e) => setGlobalFilter(e.target.value)} placeholder="Buscar..."  />
            </span>
        </div>
    );
    ...
    const productDialogFooter = (
        <>
            ...
            <Button label="Grabar" icon="pi pi-check" className="p-button-text" onClick={saveProduct} />
        </>
    );
    const deleteProductDialogFooter = (
        <>
            <Button label="Cancelar" icon="pi pi-times" className="p-button-text" onClick={hideDeleteProductDialog} />
            <Button label="Borrar" icon="pi pi-check" className="p-button-text" onClick={deleteProduct} />
        </>
    );
    const deleteProductsDialogFooter = (
        <>
            <Button label="Cancelar" icon="pi pi-times" className="p-button-text" onClick={hideDeleteProductsDialog} />
            <Button label="Borrar" icon="pi pi-check" className="p-button-text" onClick={deleteSelectedProducts} />
        </>
    );
        ...
        <div className="p-grid crud-demo">
            <div className="p-col-12">
                <div className="card">
                    ...
                    <DataTable  ref={dt} value={products} selection={selectedProducts} onSelectionChange={
                       (e) => setSelectedProducts(e.value)
                    }
                    header="Small Table" 
                        dataKey="id" paginator rows={9} rowsPerPageOptions={[5, 10, 25]} className="datatable-responsive p-datatable-sm"
                        paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                        currentPageReportTemplate="Showing {first} to {last} of {totalRecords} products"
                        globalFilter={globalFilter} emptyMessage="No existen registros" header={header} selectionMode="single" 
                        >
                    ...
                    <Dialog visible={deleteProductDialog} style={{ width: '450px' }} header="Confirmación" modal footer={deleteProductDialogFooter} onHide={hideDeleteProductDialog}>
                        <div className="confirmation-content">
                            ...
                            {product && <span>Esta seguro de borrar el registro :  <b>{product.name}</b>?</span>}
                        </div>
                    </Dialog>
